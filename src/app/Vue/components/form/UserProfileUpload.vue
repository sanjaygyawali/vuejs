<template>
<div>
		<img :src="profileLink" width="150" height="150" alt="">
		<div @click="handleClick">
		<button v-if="previousProfile" type="button" class="btn btn-default btn-block"> Update Avatar</button>
			<button v-if="!previousProfile" type="button" class="btn btn-default btn-block">Set Avatar</button>
				<span class="vicp-hint" v-show="loading !== 1"></span>
				<input type="file" v-show="false"  @change="handleChange" ref="fileinput">
		</div>
		<!-- Step On for Drag and Upload Feature -->
		...
		<!-- Show Dialog In any state  -->

<div class="vue-image-crop-upload" v-show="showDialog">
	<div class="vicp-wrap">
		<!-- Close Icon -->
		...
		<div class="vicp-step2" v-if="step == 2">
			<div class="vicp-crop">
				...
				<div class="vicp-crop-right" v-show="true">
					<div v-show="!hasError" class="vicp-preview">
						<div class="vicp-preview-item" v-if="!noSquare">
							<img :src="createImgUrl" :style="previewStyle">
							<span>Preview</span>
						</div>
						<div class="vicp-preview-item"  v-if="!noCircle">
							<img :src="createImgUrl" style="border-radius:50%!important" :style="previewStyle">
							<span>Preview</span>
						</div>
					</div>
				</div>
			...
			<div class="pull-right">
				<a class="btn green" v-show="!hasError" @click="prepareUpload" >Save</a>
			</div>
		...
		<div class="vicp-step3" v-if="step == 3">
			<div class="vicp-upload">
				<span class="vicp-loading" v-show="loading === 1"> Uploading...</span>
				...
				<div class="vicp-success" v-show="loading === 2">
					<i class="vicp-icon3"></i> Upload Success
				</div>
			</div>
			<div class="pull-right" style="margin-top:70px">
				<a @click="off" class="btn green" >Close</a>
    </div>
		</div>
...
<script>
/* eslint-disable */
import language from '@/app/Vue/utils/language.js';
import mimes from '@/app/Vue/utils/mimes.js';
import data2blob from '@/app/Vue/utils/data2blob.js';
import effectRipple from '@/app/Vue/utils/effectRipple.js';
	...
	methods: {
      ...
      this.showDialog = true
		...
		upload() {
			...
			// 	client.open('POST', '127.0.0.1/api/v1/attachment', true);
			...
			.then(response => {
				...
				if(response.status===200){
					...
					this.$emit('changeFileName', "http://127.0.0.1:8000/temp_attachment/"+ response.data.filename);
				...
				if (response.status!== 200) {
						...
						that.hasError = true;
						that.errorMsg = lang.fail;
						that.$emit('crop-upload-fail', response, field, ki);
					}
				console.log(response);
				const closeDialog= () => this.off()
